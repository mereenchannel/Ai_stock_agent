...
st.markdown("<p style='text-align: center; color: #8f9cae;'>ระบบวิเคราะห์หุ้นทางเทคนิคด้วยสถาปัตยกรรม AI <br> created by Steve R. version beta test</p>", unsafe_allow_html=True)
st.markdown("---")
...
with col_input1:
    ...
    ticker = st.text_input("🔤 ใส่สัญลักษณ์หุ้น (เช่น VOO, TSLA, PTT.BK):", value="").strip()
                ...
                if df.empty:
                    ...
                    try:
                        ...
                        if not api_key:
                            ...
                            2. วิเคราะห์แนวโน้ม: ประเมินความแข็งแกร่งของเทรนด์จากการเรียงตัวของเส้น EMA 20, 50, 200
                            3. วิเคราะห์สัญญาณ Smart Money (MCDX): ดูพฤติกรรมรายใหญ่เทียบกับเกณฑ์ฐานที่ 45
                            
                            4. การประเมินความคุ้มค่าและจังหวะเข้าซื้อ (Entry Logic & Price Action) **สำคัญมาก**:
                               - ประเมินระยะห่างระหว่าง 'ราคาปัจจุบัน' กับ 'เส้น EMA_20'
                               - หากราคาปัจจุบันอยู่ห่างจาก EMA_20 มากเกินไป (Overextended) ห้ามแนะนำให้ซื้อไล่ราคา (No FOMO) ให้แนะนำ "WAIT (รอราคาย่อตัว)"
                               - กฎการห้ามรับมีด (No Falling Knife): หากราคาร่วงลงมาแตะเส้น EMA อย่างรุนแรง (ลงหนักใน 1-2 วัน) ห้ามแนะนำให้ BUY สวนทันที แม้ว่า Risk:Reward จะดูคุ้มค่าก็ตาม ให้ฟันธง "WAIT" เพื่อรอให้ราคาหยุดตกและสร้างฐานหรือมีสัญญาณแท่งเทียนกลับตัวก่อน
                               - จะแนะนำ BUY (Market Buy) ได้ก็ต่อเมื่อ ราคาย่อตัวลงมาพักที่ EMA_20 อย่างนุ่มนวล (Consolidation) หรือเพิ่งเกิดจุดตัดทะลุแนวต้าน (Breakout) ขึ้นไปเท่านั้น

                            5. กลยุทธ์ Take Profit (Let Profit Run):
                               - ห้ามกำหนดจุด TP เป็นตัวเลขตายตัว
                               - ให้ใช้กลยุทธ์ Trailing Stop โดยจะสั่งขายก็ต่อเมื่อราคาปิดหลุดเส้น EMA_20 หรือ EMA_50
                               - แนะนำการแบ่งขายทำกำไร (Partial TP) หากพบว่าค่า MCDX ของรายใหญ่ลดลงอย่างรุนแรงจนต่ำกว่า 45 ทั้งที่ราคายังขึ้นอยู่
                               
                            6. การระบุ Action Plan: 
                               - ฟันธงคำสั่ง: BUY (Market หรือ Limit), SELL, หรือ WAIT
                               - ระบุ **"จุดเข้าซื้อ (Entry Price)"** เป็นตัวเลขให้ชัดเจน (ต้องสมเหตุสมผลกับกฎข้อ 4)
                               - ระบุ **"จุดตัดขาดทุน (Stop Loss)"** เป็นตัวเลขให้ชัดเจน (ใต้เส้น EMA_20 เล็กน้อย)
                            
                            โปรดเขียนบทวิเคราะห์ให้เฉียบคม ลึกซึ้ง และมีเหตุผลเชิงสถิติรองรับอย่างมืออาชีพ ไม่ต้องระบุสกุลเงิน (บาท/ดอลลาร์) ต่อท้ายตัวเลขราคา
                            """
